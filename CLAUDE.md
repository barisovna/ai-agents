...
### ArtifactPanel — только первый артефакт
Если в одном ответе несколько блоков кода > 8 строк — в панели открывается только первый (`artifacts[0]`). Это известное ограничение.
...
| 3 | Поддержка нескольких артефактов на одно сообщение | P2 | ⏳ Осталось |
| 4 | Виртуализация MessageList (react-virtual) при большой истории | P2 | ⏳ Осталось |
| 5 | Маршрутизация напрямую из QuickPrompts (без оркестратора) | P3 | ⏳ Осталось |
| 6 | Стриминг ответов в marketing-assistant (PHP SSE) | P2 | ⏳ Осталось |
| 7 | Добавить `.env.example` с описанием переменных | P2 | ✅ Готово |
| 8 | Обновить README.md — убрать шаблон create-next-app | P3 | ⏳ Осталось |
